<div
    x-data="{ open: false }"
    class="box-curso tema text-center relative w-full font-acephimere pt-[45px]"
    @mouseleave="open = false"
    x-transition.duration.250ms
    >
    <div class="w-full h-[45px] absolute bg-[#80009d] text-white py-2 text-center text-xl font-bold  top-0" x-transition.duration.250ms x-show="open">¡ME INSCRIBO!</div>
    <div class="shadow-2xl" @mouseover="open = true" x-transition.duration.250ms>
        ...
        <div class="text-left principal bg-[#ff2d00] py-4">
            <div class="flex justify-end relative z-10">
                <h3 class="h-[84px] text-xl font-bold px-4 transition-all w-2/3  text-white text-right">{{ $curso->nombre}}</h3>
            </div>
            <div style="font-size: 13px;line-height: 1.3em;" class="relative z-10 px-4 text-white italic text-right pl-12 transition-all" x-transition.duration.250ms x-show="open">{{ $curso->descripcion_corta }}</div>
