<div class="shadow p-2 mt-3 mb-3">
    @forelse( $seccion['contenido'] as $keyc => $contenido )
        <div class="shadow p-2 mt-3 mb-3 hover:border-2 rounded-lg hover:border-[#6b2b83]" x-data="{ open: false }">
            <label class="flex justify-center items-center text-white rounded-lg text-sm font-bold  text-center py-3 bg-[#6b2b83]" @click="open = !open">Contenido
                #{{ $loop->index + 1 }}<span x-show="open" class="ml-4">@include('livewire.cursos._icons.arrow_up')</span><span x-show="!open" class="ml-4">@include('livewire.cursos._icons.arrow_down')</span></label>
            <div x-show="open" x-transition.duration.500ms >
                ...
                <div class="box-entrada px-2 w-full mt-2">
                    ...
                    <select id="countries"
                        ...
                        wire:model.lazy="secciones.{{ $key }}.contenido.{{ $keyc }}.cursos_contenido_tipo_id">
                ...
                <div class="box-entrada px-2 w-full mt-2">
                  <label for="countries" class="block text-gray-700 text-sm font-bold mt-2">Video(url)</label>
                  <input type="text"
                        class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 w-full block rounded-none rounded-r-md sm:text-sm border-gray-300"
                        placeholder="url..."
                        wire:model.lazy="secciones.{{ $key }}.contenido.{{ $keyc }}.recurso">
                        ...
                        <div class="" style="width: fit-content">
                            @if ($secciones[$key]['contenido'][$keyc]['img_fondo'])
                                <div class="my-2 mx-auto relative" style="width: 75px">
                                    ...
                                    <img src="{{ $secciones[$key]['contenido'][$keyc]['img_fondo']->temporaryUrl() }}"
                ...
                <div class="flex mt-3">
                  <div class="rounded-lg bg-red-600 p-2" title="Eliminar Sección" wire:click="borrarContenido({{ $key }}, {{ $keyc }})">
    ...
    <div class="flex justify-end">
        <x-jet-button wire:click="agregar_contenido('{{ $key }}')" type="button"
