<div class="shadow p-2 mt-5 hover:border-2 hover:border-[#6b2b83]">
    ...
    @forelse( $secciones as $key => $seccion )
        <div class="shadow p-2 mt-3 mb-3 hover:border-2 hover:border-[#6b2b83] rounded-lg" x-data="{ open: false }">
          <label class="flex justify-center items-center bg-[#6b2b83] py-3 text-sm font-bold rounded-lg text-center text-white " @click="open = !open">Sección #{{ $loop->index + 1 }}<span x-show="open" class="ml-4">@include('livewire.cursos._icons.arrow_up')</span><span x-show="!open" class="ml-4">@include('livewire.cursos._icons.arrow_down')</span></label>
          <div x-show="open" x-transition.duration.500ms>
            ...
            @include('livewire.cursos._partes._form._editar.form_contenidos')
            <div class="flex">
                <div class="rounded-lg bg-red-600 p-2" title="Eliminar Sección" onclick="confirm('Esta seguro que desea eliminar esta sección.') || event.stopImmediatePropagation()"wire:click="borrarSeccion({{ $key }})">
