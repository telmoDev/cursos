<div x-data="{ scroll: (elemento) => { avance = document.getElementsByTagName('header')[0].offsetHeight + (window.innerHeight * elemento);
        console.log(avance);
        window.scrollTo({ top: avance, behavior: 'smooth' }); } }" x-init="{ a:  }">
    <div class="">
      @foreach ($contenido as $item)
        <div class="flex flex-col h-screen justify-center" id="introduccion">
          <div class="flex flex-col">
              <div class="text-center">
                <div class="border-8 py-5 px-8 text-center text-5xl font-bold uppercase text border-[#6b2b83] inline-block text-[#6b2b83]">Introducción
                </div>
              </div>
                <div class="text-center  text-2xl font-bold mt-4"> Innovación de modelos de negocios</div>
                @if ($item->contenido_download )
                  <div class="text-center my-2">
                    <a href="{{ route('contenido.download.file', [ 'folder' => $curso->id, 'folder2' => $item->id, 'filename' => $item->contenido_download ]) }}" download class="flex items-center justify-center">
                ...
                <div class="text-center  text-2xl font-bold mt-4">
                    <x-jet-secondary-button type="text" class="bg-[#6b2b83] text-white hover:text-white hover:opacity-90 active:bg-[#6b2b83] focus:border-none focus:shadow-none active:text-white" @click="scroll({{ $loop->index + 1 }})" @keydown.enter="alert('Submitted!')">Siguiente</x-jet-secondary-button>
                </div>
                <div class="text-center flex justify-center items-center text-sm font-bold mt-4"><span class="mr-1">@include('livewire.cursos._icons.clock')</span>Toma 5 min</div>
            </div>
        </div>
      @endforeach
        {{-- <div class="flex flex-col h-screen justify-center">
            <div class="flex flex-col">
                ...
                <div class="text-center  text-2xl font-bold mt-4">
                    <x-jet-secondary-button @click="scroll(2)">Siguiente</x-jet-secondary-button>
                </div>
            </div>
        </div>
        <div class="flex flex-col h-screen justify-center">
            <div class="flex flex-col">
                <div class="text-center  text-2xl font-bold mt-4">Mientras más conocimiento una persona tenga sobre algo
                    o alguien, más poder tendrá.</div>
                <div class="text-center  text-2xl font-bold mt-4">
                    <x-jet-secondary-button @click="scroll(3)">Comenzar</x-jet-secondary-button>
                </div>
            </div>
        </div> --}}
        @if (true)
          <div class="flex cursor-pointer fixed right-8" style="top: 90%">
            ...
            <div class="bg-[#6b2b83] rounded-r-md p-3 cursor-pointer hover:opacity-90" style="border-left: .5px solid #DB3634">
