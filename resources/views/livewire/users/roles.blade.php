            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                @foreach ($objects as $object)
                    <tr class="text-gray-700 dark:text-gray-400">
                        ...
                        @endforeach
                        <td class="px-4 py-3">
                            @foreach( $object->permissions as $permiso )
                                <span class="text-white inline-flex items-center justify-center px-2 py-1 mr-2 text-xs font-bold leading-none  bg-red-600 rounded-full">
                                    {{ $permiso->name  }}
                                </span>
                            @endforeach
                        </td>
    ...
    <x-jet-confirmation-modal wire:model="popup">
        ...
        <x-slot name="content">
                ...
                <table class="w-full whitespace-no-wrap">
                        ...
                        @endforeach
                            <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                                <th class="px-4 py-3">Permisos</th>
                                <td class="px-4 py-3">
                                    @foreach( $elemento->permissions as $permiso )
                                        <span class=" mb-2 text-white inline-flex items-center justify-center px-2 py-1 mr-2 text-xs font-bold leading-none  bg-red-600 rounded-full">
                                            {{ $permiso->name  }}
                                        </span>
                                    @endforeach
                                </td>
            ...
            @if( $popup_tipo == "Editar" )
                @include("livewire.users._partes.roles.formulario")
            @endif
            @if( $popup_tipo == "Crear" )
                @include("livewire.users._partes.roles.formulario")
            @endif
